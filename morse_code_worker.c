...
const unsigned char symbol_array[43][3] = {
    "А", "Б", "В", "Г", "Д", "Е", "Ж", "З", "И", "Й",
    "К", "Л", "М", "Н", "О", "П", "Р", "С", "Т", "У",
    "Ф", "Х", "Ц", "Ч", "Ш", "Щ", "Ъ", "Ы", "Ь", "Э",
    "Ю", "Я",
   ...
   if(furi_string_size(instance->words) > 63) furi_string_reset(instance->words);
    for(size_t i = 0; i < sizeof(morse_array)/sizeof(morse_array[0]); i++) {
        if(furi_string_cmp_str(instance->buffer, morse_array[i]) == 0) {
            furi_string_push_back(instance->words, symbol_array[i][0]);
            if(i < 32) { // For Russian characters only
                furi_string_push_back(instance->words, symbol_array[i][1]);
            }
            break;
        }
    }
